...
describe('OtpComponent (Vérification OTP)', () => {
 ...
 mockVerifyService.verifyOtp.calls.reset(); mockVerifyService.resendOtp.calls.reset(); mockRouter.navigate.calls.reset(); mockToastrService.error.calls.reset(); mockToastrService.warning.calls.reset();  component.ngOnInit(); fixture.detectChanges(); });
 ...
 component.submitOtp();  component.submitOtp();  expect(component.isBlocked).toBe(false);
 component.submitOtp();  tick(); 
 expect(component.attempts).toBe(0);  expect(component.isBlocked).toBe(true); 
